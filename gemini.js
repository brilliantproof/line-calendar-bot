...
可能的意圖：
1. create（新增行程）：包含具體時間和事件，或包含明確的未來日期（即使沒有時鐘時間，也算——見下方「整天行程」規則）
2. query（查詢行程）：詢問某段時間有什麼「行程」
3. cancel（取消行程）：要取消或刪除某個行程
4. need_info（用戶想建立行程，但連日期都沒有提供）
5. query_notes（查詢筆記）：用戶用問句要求回顧/查看自己之前記錄的筆記內容（不是行程）
6. null（與上述皆無關，包含單純的工作記錄、心得、進度更新等「回顧已完成事項的陳述句」——這些本身就是一則筆記，不是要求查詢）
...
create 的關鍵條件：每一項都必須有明確的日期線索（例如「三點」「下午」「明天」「這週五」「8/21」）。
即使句子裡提到「會議」「報告」「開會」這類聽起來像行程的字眼，只要沒有任何日期線索，就一律是 null，
不要硬湊成 create——那是在描述已經做完的事，不是要排時間。
- 「今天完成了電子報、開了影響力概念會議、完成了財務報告」→ null（沒有任何日期線索，這是回顧今天已完成的工作記錄）
- 「明天下午三點開影響力概念會議」→ create（有明確時間，一般行程）

整天行程（all-day）規則：如果訊息提到一個明確日期（未來或今天都算），但沒有提供時鐘時間，
且是在標記「某件事會在/到那天發生、結束、開始、截止」（而不是回顧已經完成的事），
就視為 create，但 time 填 null、並加上 "allDay":true（不要因為沒時間就丟到 need_info 或 null）。
- 「8/21暑輔結束」→ create，allDay（有明確日期 8/21，標記的是未來會發生的事，不是回顧）
- 「這週五交報告」→ create，allDay（有日期，沒有時鐘時間，標記截止日）
- 「今天完成了電子報」→ null（雖然有「今天」，但描述的是已完成的事，不是未來標記）
- 「請幫我建立行程」「新增一個行程」→ need_info（連日期都沒有）
...
整天行程（只有日期、沒有時鐘時間）：
{"action":"create","title":"暑輔結束","date":"2026-08-21","time":null,"endTime":null,"location":null,"allDay":true}
